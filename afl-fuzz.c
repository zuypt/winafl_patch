...
static u8  virgin_tmout[MAP_SIZE],    /* Bits we haven't seen in tmouts   */
...
static void setup_shm(void) {
  HANDLE hMap;
  char *shm_str, *map_str, *var_str, *virgin_str, *module_str;
  ...
  while(attempts < 5) {
    ...
    map_str = (char *)alloc_printf("afl_shm_%s_MAP", fuzzer_id);
    var_str = (char *)alloc_printf("afl_shm_%s_VAR", fuzzer_id);
    virgin_str = (char *)alloc_printf("afl_shm_%s_VIRGIN", fuzzer_id);
    module_str = (char *)alloc_printf("afl_shm_%s_MOD", fuzzer_id);
    ...
    hMap = CreateFileMapping(
                  ...
                  PAGE_READWRITE,
                  0,
                  sizeof(module_t)*MAX_COV_MODULE,
                  module_str);

    if (hMap == NULL) {
      PFATAL("CreateFileMapping for MOD_INFO failed");
    }

    // We don't need to map the shared MAP
    hMap = CreateFileMapping(
                  INVALID_HANDLE_VALUE,
                  NULL,
                  PAGE_READWRITE,
                  0,
                  MAP_SIZE*sizeof(void*),
                  map_str);

    if (hMap == NULL) {
      PFATAL("CreateFileMapping for MAP failed");
    }
    ...
    hMap = CreateFileMapping(
                   ...
                   0,                       // maximum object size (high-order DWORD)
                   MAP_SIZE,                // maximum object size (low-order DWORD)
                   (char *)var_str);        // name of mapping object

    if (hMap == NULL) {
      PFATAL("CreateFileMapping for MAP failed");
    }

    var_bytes = (u8 *)MapViewOfFile(
    ...
    if (!var_bytes) PFATAL("MapViewOfFile var_bytes failed");
    ...
    hMap = CreateFileMapping(
                   ...
                   0,                       // maximum object size (high-order DWORD)
                   MAP_SIZE,                // maximum object size (low-order DWORD)
                   (char *)virgin_str);        // name of mapping object

    if (hMap == NULL) {
      PFATAL("CreateFileMapping for MAP failed");
    }

    virgin_bits = (u8 *)MapViewOfFile(
      hMap,          // handle to map object
      FILE_MAP_ALL_ACCESS, // read/write permission
      0,
      0,
    ...
    if (!virgin_bits) PFATAL("MapViewOfFile virgin_bits failed");
