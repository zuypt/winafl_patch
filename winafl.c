...
static dr_emit_flags_t
instrument_bb_coverage2(void *drcontext, void *tag, instrlist_t *bb, instr_t *inst,
                      bool for_trace, bool translating, void *user_data)
{
    static bool debug_information_output = false;
    app_pc start_pc;
    ...
    dr_printf("compile");
    for(blk_id=0; blk_id<BLK_ID; blk_id++) {
      if (MAP[blk_id] == start_pc) {
        found = true;
        break;
      }
    }

    if (found) offset = blk_id;
    else {
      MAP[BLK_ID] = start_pc;
      offset = BLK_ID;
      BLK_ID += 1;
      if (BLK_ID >= MAP_SIZE)
      {
        char errorMessage[] = "increase MAP_SIZE";
...
event_module_load(void *drcontext, const module_data_t *info, bool loaded)
{
    ...
    if (options.coverage_kind == COVERAGE_BB2) {
        target_modules = options.target_modules;
        while(target_modules) {
            if(_stricmp(module_name, target_modules->module_name) == 0) {
                for(int i=0; i<MAX_COV_MODULE; i++) {
                    if (MODULE_INFO[i].start == NULL) {
                      strcpy(MODULE_INFO[i].name, module_name);
                      MODULE_INFO[i].start = (void *)info->start;
                      MODULE_INFO[i].end = (void *)info->end;
                      break;
                    } else {
                      // Already in MODULE_INFO
                      if (MODULE_INFO[i].start == (void *)info->start) break;
                    }
                }
            }
            target_modules = target_modules->next;
...
setup_map()
{
  char map_name[MAXIMUM_PATH];
  char mod_info_name[MAXIMUM_PATH];

  strcpy(map_name, options.shm_name);
  strcat(map_name, "_MAP");

  strcpy(mod_info_name, options.shm_name);
  strcat(mod_info_name, "_MOD");
  ...
  map_file = OpenFileMapping(
   ...
   if (map_file == NULL) DR_ASSERT_MSG(false, "error accesing shared MAP");

   MAP =  MapViewOfFile(map_file, // handle to map object
               FILE_MAP_ALL_ACCESS,  // read/write permission
               0,
               0,
               MAP_SIZE*sizeof(app_pc));

   if (MAP == NULL) DR_ASSERT_MSG(false, "error accesing shared MAP");

  map_file = OpenFileMapping(
                   FILE_MAP_ALL_ACCESS,   // read/write access
                   FALSE,                 // do not inherit the name
                   mod_info_name);        // name of mapping object

   if (map_file == NULL) DR_ASSERT_MSG(false, "error accesing shared MODULE_INFO");

   MODULE_INFO =  MapViewOfFile(map_file, // handle to map object
               FILE_MAP_ALL_ACCESS,  // read/write permission
               0,
               0,
               sizeof(module_t)*4);

   if (MODULE_INFO == NULL) DR_ASSERT_MSG(false, "error accesing shared MODULE_INFO");

    BLK_ID = 0;
    while (1)
    {
      if (MAP[BLK_ID] == 0) break;
      BLK_ID += 1;
    }
}
  ...
  options.dr_persist_cache = false;
    ...
    for (i = 1/*skip client*/; i < argc; i++) {
        ...
        if (strcmp(token, "-no_nudge_kills") == 0)
            ...
            else if(strcmp(token, "bb2")==0) options.coverage_kind = COVERAGE_BB2;
...
dr_client_main(client_id_t id, int argc, const char *argv[])
{
    ...
    } else if (options.coverage_kind == COVERAGE_BB2) {
        drmgr_register_bb_instrumentation_event(NULL, instrument_bb_coverage2, NULL);
    }
